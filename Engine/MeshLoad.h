...
public:
	...
	static std::vector<std::shared_ptr<Mesh>> loadMeshFile(GameObject& parentGO, const std::string& path)
	{
		...
		auto scene = aiImportFile(path.c_str(), aiProcess_Triangulate | aiProcess_FlipUVs /*| aiProcess_GenNormals*/);
		for (size_t m = 0; m < scene->mNumMeshes; ++m) {
			...
			for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
				...
				mesh_ptr->meshNormals.push_back(glmNormal);
			}

			for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
				aiVector3D vert = mesh->mVertices[i];
				vec3f glmNormal(vert.x, vert.y, vert.z);
				mesh_ptr->meshVertices.push_back(glmNormal);
			}
			...
			for (unsigned int i = 0; i < mesh->mNumFaces; i++) {
				...
				mesh_ptr->meshFaceNormals.push_back(faceNormal);
